...
exports.dashboardInfo = async () => {
    let transacions = await db.query("SELECT \n" +
        "(SELECT sum(amount) FROM transact WHERE transactionDate BETWEEN date(CURRENT_TIMESTAMP, '-1 MONTH' ) AND CURRENT_TIMESTAMP) as 'month',\n" +
        "(SELECT sum(amount) FROM transact WHERE transactionDate BETWEEN date(CURRENT_TIMESTAMP, '-1 day' ) AND CURRENT_TIMESTAMP) as 'day'");
    return transacions;
};